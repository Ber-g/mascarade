...
tempo_interval = 60 / tempo_tracker.get_tempo(duration=5)  # Intervalle de temps entre chaque battement de tempo (120 BPM)
offset_range = 30  # Décalage maximum en pixels pour les lignes rouges

def update_tempo():
    global tempo_interval, last_tempo_update
    while True:
        time.sleep(30)
        tempo_interval = 60 / tempo_tracker.get_tempo(duration=30)
        last_tempo_update = time.time()

# Démarrer le thread pour mettre à jour le tempo
threading.Thread(target=update_tempo, daemon=True).start()
    ...
    while cap.isOpened():
        ...
        if pose_results.pose_landmarks:
            for (start_idx, end_idx) in BODY_CONNECTIONS:
                ...
                cv2.line(black_frame, start_point, end_point, (255, 0, 0), 8)
        ...
        if face_results.multi_face_landmarks:
            for face_landmarks in face_results.multi_face_landmarks:
                for (start_idx, end_idx) in FACE_CONNECTIONS:
                    ...
                    start_point = int(start_landmark.x * black_frame.shape[1]), int(start_landmark.y * black_frame.shape[0])
                    end_point = int(end_landmark.x * black_frame.shape[1]), int(end_landmark.y * black_frame.shape[0])
                    cv2.line(black_frame, start_point, end_point, (75, 200, 12), 1)
